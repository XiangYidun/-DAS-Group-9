...
author: "Diwen Jiang, Wei Xiang, Zilu Liu"
format:
  pdf:
    fontsize: 12pt
...
At this stage, we can clearly observe that the **length** variable contains some missing values. No other obvious anomalies are present in the data set. However, further discussion and analysis are needed.
...
## Check whether the observations are inpendent
...
From the Residuals vs Fitted plot, we can see that the model may have sysmetric errors, and the fitting effect is not good at extreme values (close to 0 or 1).
